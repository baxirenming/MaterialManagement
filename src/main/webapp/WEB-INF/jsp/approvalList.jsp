...
<body>
	...
	<div>
	   From:<input type="date" name="form.dateFrom" value="<s:property value="form.dateFrom"/>" />
	   To:<input type="date" name="form.dateTo" value="<s:property value="form.dateTo"/>" />
	...
	<tr>
		...
		<td>
			<s:if test="status == 1 || status == 3">
				<a href="/sizaikanri/approvalList/approval?form.target_id=<s:property value="id"/>">承認</a>
			</s:if>
		</td>
		<td>
			<s:if test="status == 1">
				<a href="/sizaikanri/approvalList/regect?form.target_id=<s:property value="id"/>">却下</a>
			</s:if>
		</td>
	</tr>
	</s:iterator>
